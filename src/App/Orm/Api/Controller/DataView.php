...
class DataView extends Controller
{
    ...
    public function getObjectName(): string
    {
        ...
        if (!$dataObject || !$this->orm->configExists($dataObject)) {
            return 'Orm';
        }
    ...
    public function viewConfigAction()
    {
        ...
        if (empty($shard) && $objectConfig->isDistributed()) {
            ...
            $title .= ' INDEX';
            $fields = $this->orm->config($objectConfig->getDistributedIndexObject())->getFields();
    ...
    public function editorConfigAction()
    {
        ...
        foreach ($objectFieldList as $field) {
            ...
            if ($fieldObj->isMultiLink()) {
                ...
                $linkedCfg = $this->orm->config($linkedObject);
                $related[] = array(
                    'field' => $field,
                    'object' => $linkedObject,
                    ...
                    $data[] = '
						{
							...
							isVc:' . (int)($objectConfig->isRevControl()) . '
						}
					';
                }
            } else {
